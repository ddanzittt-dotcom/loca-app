...
var features = [
  { id: "f1", mapId: "m1", type: "pin", title: "블루보틀 성수", category: "_none", tags: ["커피","디저트"], emoji: "☕", isHl: true, geojson: { type: "Feature", geometry: { type: "Point", coordinates: [127.056,37.544] }, properties: {} }, logs: [{ id: "l1", date: new Date().toISOString().slice(0,10), note: "오트밀크 라떼가 최고!", mood: "😋" }] },
...
var activeMapId = "m1", roles = {}, selectedId = null, drawMode = null, draftPts = [];
var filterCats = [], currentTab = "map", listSearchMode = "all";
...
var posterMapInst = null, selectedFrame = 'default', frameCatFilter = 'all';
...
function toast(msg, type) {
  var c = type === 's' ? 'var(--success)' : type === 'e' ? 'var(--danger)' : type === 'w' ? 'var(--warn)' : '#555';
  var el = document.createElement('div'); el.className = 'toast'; el.style.background = c; el.textContent = msg;
  document.getElementById('toasts').appendChild(el);
  setTimeout(function () { el.classList.add('out'); setTimeout(function () { el.remove(); }, 220); }, 2200);
}
...
// ── STORAGE ──
function loadData() { try { var s = localStorage.getItem('loca_v2'); if (s) { var p = JSON.parse(s); if (p.maps && p.maps.length) { maps = p.maps; features = p.features || []; CATS = p.cats || []; activeMapId = p.activeMapId || maps[0].id; roles = p.roles || {}; } } } catch (e) { } }
...
function applyPermissionUI() {
  var obs = isObserver();
  document.querySelectorAll('.editor-only').forEach(function (el) { el.style.display = obs ? 'none' : 'flex'; });
  document.getElementById('observer-map-banner').style.display = obs ? 'block' : 'none';
  var am = getActiveMap(); var si = document.getElementById('sync-indicator');
  if (am && am.kind === 'our') { si.style.display = 'flex'; si.innerHTML = '<div style="width:8px;height:8px;border-radius:4px;background:#22C55E;animation:syncPulse 2s infinite"></div>'; } else si.style.display = 'none';
  var sr = document.getElementById('stats-role');
  if (am && am.kind === 'our') { sr.style.display = 'block'; sr.innerHTML = roleBadgeHtml(getRole(am.id) || 'owner'); } else sr.style.display = 'none';
}

// ══════ CATEGORY SYSTEM ══════
function getFilterableCats() {
  var all = CATS.slice();
  var hasNone = features.some(function (f) { if (f.mapId !== activeMapId) return false; return !f.category || f.category === '_none' || f.category === 'unknown' || !CATS.some(function (c) { return c.id === f.category; }); });
...
function buildCatGrid(activeCat) {
  ...
  for (var ci = 0; ci < 7; ci++) {
    if (ci < CATS.length) { var c = CATS[ci]; h += '<div class="cat-btn ' + (activeCat === c.id ? 'on' : '') + '" data-cid="' + c.id + '" data-action="selectcat"><div class="e">' + c.emoji + '</div><div class="l">' + c.label + '</div></div>'; }
    else { h += '<div class="cat-btn" data-action="addcat" style="border-style:dashed;opacity:.6"><div class="e" style="font-size:15px">➕</div><div class="l">추가</div></div>'; }
  }
...
function buildEmojiPicker(sel) {
  ...
  for (var i = 0; i < CAT_EMOJIS.length; i++) { var em = CAT_EMOJIS[i], s = em === sel; h += '<div class="emoji-pick" data-eidx="' + i + '" style="width:36px;height:36px;border-radius:12px;border:2px solid ' + (s ? 'var(--pri)' : 'var(--bdr)') + ';background:' + (s ? 'var(--pri-lt)' : '#fff') + ';display:flex;align-items:center;justify-content:center;font-size:18px;cursor:pointer">' + em + '</div>'; }
  h += '</div>'; return h;
}
function setDetCat(c) { _detCat = c; document.querySelectorAll('#det-cats .cat-btn').forEach(function (el) { var cid = el.getAttribute('data-cid'); if (cid) el.classList.toggle('on', cid === c); }); }
function showAddCatSheet() {
  _addCatEmoji = '📍'; _editingCatId = null; var body = document.getElementById('catmgr-body');
  var h = '<div style="font-size:15px;font-weight:900;margin-bottom:10px">➕ 새 카테고리 만들기</div>';
  if (CATS.length >= MAX_CATS) { h += '<div style="text-align:center;padding:20px;color:var(--danger);font-size:13px;font-weight:700">최대 ' + MAX_CATS + '개까지</div><button class="btn btn-out btn-block mt-8" onclick="closeSheet(\'catmgr\')">닫기</button>'; body.innerHTML = h; openSheet('catmgr'); return; }
  ...
  h += '<button class="btn btn-pri btn-block mt-14" onclick="confirmAddCat()">✨ 만들기</button><button class="btn btn-out btn-block mt-8" onclick="closeSheet(\'catmgr\')">취소</button>';
...
function confirmAddCat() {
  ...
  if (CATS.length >= MAX_CATS) { toast('최대 ' + MAX_CATS + '개', 'w'); return; }
  if (CATS.some(function (c) { return c.label === name; })) { toast('이미 있는 이름', 'w'); return; }
  CATS.push({ id: uid('cat'), label: name, emoji: _addCatEmoji }); save(); closeSheet('catmgr');
  toast(_addCatEmoji + ' ' + name + ' 추가!', 's');
...
function showCatManager() {
  ...
  var h = '<div style="font-size:15px;font-weight:900;margin-bottom:4px">⚙️ 카테고리 관리</div><div style="font-size:11px;color:var(--mut);font-weight:700;margin-bottom:12px">' + CATS.length + '/' + MAX_CATS + '개</div>';
  if (!CATS.length) { h += '<div style="text-align:center;padding:30px;color:var(--mut)"><div style="font-size:28px">📂</div><div style="font-size:12px;font-weight:700;margin-top:6px">카테고리가 없어요</div></div>'; }
  else { for (var i = 0; i < CATS.length; i++) { var c = CATS[i], fc = 0; for (var j = 0; j < features.length; j++) { if (features[j].category === c.id) fc++; } h += '<div class="card" style="margin-bottom:8px;padding:10px 12px"><div style="display:flex;align-items:center;gap:10px"><div style="width:40px;height:40px;border-radius:14px;background:var(--pri-lt);display:flex;align-items:center;justify-content:center;font-size:18px">' + c.emoji + '</div><div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:900">' + c.label + '</div><div style="font-size:10px;color:var(--mut);font-weight:700;margin-top:1px">📍 ' + fc + '개</div></div><button class="btn btn-out" style="padding:6px 10px;font-size:10px" data-action="editcat" data-cid="' + c.id + '">✏️</button><button class="btn btn-danger" style="padding:6px 10px;font-size:10px" data-action="deletecat" data-cid="' + c.id + '">🗑</button></div></div>'; } }
  if (CATS.length < MAX_CATS) h += '<button class="btn btn-sec btn-block mt-8" onclick="showAddCatSheet()">➕ 추가</button>';
  h += '<button class="btn btn-out btn-block mt-8" onclick="closeSheet(\'catmgr\')">닫기</button>'; body.innerHTML = h; openSheet('catmgr');
}
function showEditCat(cid) {
  var c = null; for (var i = 0; i < CATS.length; i++) { if (CATS[i].id === cid) { c = CATS[i]; break; } } if (!c) return;
  _addCatEmoji = c.emoji; _editingCatId = cid; var body = document.getElementById('catmgr-body');
  var h = '<div style="font-size:15px;font-weight:900;margin-bottom:10px">✏️ 카테고리 수정</div>' + buildEmojiPicker(c.emoji);
  h += '<div class="label mt-12">카테고리 이름</div><input class="input" id="addcat-name" value="' + c.label + '" maxlength="10">';
  h += '<button class="btn btn-pri btn-block mt-14" onclick="confirmEditCat()">💾 저장</button><button class="btn btn-out btn-block mt-8" onclick="showCatManager()">← 뒤로</button>';
  body.innerHTML = h;
}
function confirmEditCat() {
  if (!_editingCatId) return; var c = null; for (var i = 0; i < CATS.length; i++) { if (CATS[i].id === _editingCatId) { c = CATS[i]; break; } } if (!c) return;
  var name = (document.getElementById('addcat-name').value || '').trim(); if (!name) { toast('이름 필요', 'w'); return; }
  c.label = name; c.emoji = _addCatEmoji; _editingCatId = null; save(); toast('수정 완료 ✅', 's'); showCatManager(); renderMapFeatures();
...
function deleteCat(cid) {
  ...
  showConfirm('삭제', c.emoji + ' ' + c.label + ' 삭제?' + (fc ? ' (' + fc + '개→미분류)' : ''), function () {
    for (var k = 0; k < features.length; k++) { if (features[k].category === cid) features[k].category = '_none'; }
    CATS = CATS.filter(function (x) { return x.id !== cid; }); filterCats = filterCats.filter(function (x) { return x !== cid; });
...
document.addEventListener('click', function (e) {
  ...
  else if (act === 'pickframe') pickFrame(btn.dataset.fid);
});
document.addEventListener('click', function (e) {
  var pick = e.target.closest('.emoji-pick'); if (!pick) return;
  var idx = parseInt(pick.getAttribute('data-eidx')); if (isNaN(idx) || !CAT_EMOJIS[idx]) return;
...
// ══════ LIST TAB ══════
function setListSearchMode(mode) { listSearchMode = mode; document.querySelectorAll('#search-mode-tabs .chip').forEach(function (c) { c.classList.toggle('on', c.dataset.smode === mode); }); var inp = document.getElementById('list-search'); var ph = { all: '제목·메모·태그·카테고리 통합 검색', title: '제목으로 검색', memo: '메모 내용으로 검색', tag: '태그로 검색', category: '카테고리로 검색' }; inp.placeholder = ph[mode] || '검색…'; renderList(); }
...
function showImportSheet() {
  ...
  h += '<div style="font-size:11px;color:var(--mut);font-weight:700;margin-bottom:12px">지도 URL 파싱</div>';
  h += '<div class="card" style="margin-bottom:10px"><div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><div style="font-size:18px">🔗</div><div><div style="font-size:13px;font-weight:900">지도 URL 붙여넣기</div></div></div>';
  h += '<textarea class="input" id="import-urls" rows="8" placeholder="URL을 한 줄에 하나씩 붙여넣기\n\n예시:\nhttps://map.naver.com/...?c=127.056,37.544,15\nhttps://map.kakao.com/link/map/블루보틀,37.544,127.056\nhttps://google.com/maps/place/.../@37.544,127.038,16z\n37.544, 127.056" style="font-size:11px;line-height:1.5"></textarea>';
  h += '<div class="label mt-8">지도 이름</div><input class="input" id="import-map-name" placeholder="예: 성수동 맛집">';
  h += '<button class="btn btn-pri btn-block mt-8" onclick="startUrlImport()">🔍 URL 파싱</button></div>';
...
function parseMapUrl(line) {
  ...
  var r = null;
  var gAt = line.match(/@(-?\d+\.?\d+),(-?\d+\.?\d+)/); if (gAt) { var gn = ''; var gp = line.match(/\/place\/([^/@]+)/); if (gp) gn = decodeURIComponent(gp[1].replace(/\+/g, ' ')); r = { lat: parseFloat(gAt[1]), lng: parseFloat(gAt[2]), name: gn, source: '구글맵' }; }
...
function startUrlImport() {
  var raw = document.getElementById('import-urls').value.trim(); if (!raw) { toast('URL 입력', 'w'); return; }
  importMapName = document.getElementById('import-map-name').value.trim() || '가져온 지도';
  var lines = raw.split('\n').map(function (l) { return l.trim(); }).filter(function (l) { return l.length > 0; }); if (!lines.length) { toast('URL 입력', 'w'); return; }
  var items = [], nameHints = {};
  for (var i = 0; i < lines.length; i++) { var ln = lines[i]; if (ln.indexOf('http') < 0 && !ln.match(/\d{2,}\.\d{3,}/)) { var clean = ln.trim(); if (clean && i + 1 < lines.length) { nameHints[i + 1] = clean; } continue; } items.push({ line: ln, idx: i, nameHint: nameHints[i] || '' }); }
  if (!items.length) { toast('파싱 가능한 URL 없음', 'w'); return; }
  importResults = [];
  items.forEach(function (it) { var parsed = parseMapUrl(it.line); if (parsed) { if (!parsed.name && it.nameHint) parsed.name = it.nameHint; importResults.push({ lat: parsed.lat, lng: parsed.lng, name: parsed.name || '', found: true, original: it.line, source: parsed.source, category: '_none' }); } else { importResults.push({ lat: 0, lng: 0, name: it.nameHint || '', found: false, original: it.line, source: '실패', category: '_none' }); } });
  var pg = document.getElementById('import-progress'); pg.style.display = 'block';
  pg.innerHTML = '<div style="background:var(--srf);border:1px solid var(--bdr);border-radius:16px;padding:12px"><div style="display:flex;align-items:center;gap:8px"><div style="font-size:16px;animation:syncPulse 1s infinite">🔍</div><div style="font-size:13px;font-weight:900">주소 조회 중…</div></div><div style="height:6px;background:var(--bdr);border-radius:3px;overflow:hidden;margin-top:8px"><div id="gc-bar" style="height:100%;background:var(--pri);border-radius:3px;width:10%;transition:width .3s"></div></div><div id="gc-status" style="font-size:10px;color:var(--mut);font-weight:700;margin-top:6px"></div></div>';
  document.getElementById('import-results').style.display = 'none';
  reverseQueue = []; reverseTotal = 0;
  importResults.forEach(function (r, i) { if (r.found && !r.name) reverseQueue.push(i); }); reverseTotal = reverseQueue.length;
...
function processNextReverse() {
  ...
  var idx = reverseQueue.shift(); var r = importResults[idx]; var done = reverseTotal - reverseQueue.length;
  document.getElementById('gc-bar').style.width = Math.round(10 + done / reverseTotal * 90) + '%';
  document.getElementById('gc-status').textContent = '주소 조회 (' + done + '/' + reverseTotal + ')';
  fetch('https://nominatim.openstreetmap.org/reverse?lat=' + r.lat + '&lon=' + r.lng + '&format=json&accept-language=ko&zoom=18')
    .then(function (resp) { return resp.json(); }).then(function (data) { if (data && data.display_name) { var parts = (data.display_name || '').split(','); r.name = r.name || (parts[0] || '').trim(); } if (!r.name) r.name = '장소 ' + (idx + 1); setTimeout(processNextReverse, 350); })
...
function finishUrlImport() {
  ...
  var h = '<div style="background:var(--srf);border:1px solid var(--bdr);border-radius:16px;padding:12px"><div style="font-size:13px;font-weight:900;margin-bottom:8px">결과</div>';
  ...
  h += '</div>';
  if (found.length) { h += '<div style="max-height:200px;overflow-y:auto;margin-bottom:8px">'; found.forEach(function (r) { h += '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-top:1px solid var(--bdr)"><div>📍</div><div style="flex:1;min-width:0"><div style="font-size:11px;font-weight:900;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">' + (r.name || '장소') + '</div><div style="font-size:9px;color:var(--mut);font-weight:700">' + r.source + ' · ' + r.lat.toFixed(4) + ', ' + r.lng.toFixed(4) + '</div></div><div style="font-size:9px;color:var(--success);font-weight:900">✅</div></div>'; }); h += '</div>'; h += '<button class="btn btn-pri btn-block" onclick="confirmUrlImport()">📍 ' + found.length + '개 추가</button>'; }
  else h += '<div style="text-align:center;color:var(--danger);font-size:12px;font-weight:700;padding:8px">파싱 실패</div>';
...
function confirmUrlImport() {
  ...
  var m = { id: uid('m'), kind: 'my', title: importMapName, desc: found.length + '개', theme: THEMES[Math.floor(Math.random() * THEMES.length)], ver: 1 }; maps.push(m); activeMapId = m.id;
  ...
  save(); closeSheet('import'); renderMapFeatures(); fitToFeatures(); updateStats(); applyPermissionUI(); toast(importMapName + ' 추가!', 's');
}
function importFile(e) { var file = e.target.files[0]; if (!file) return; var r = new FileReader(); r.onload = function (ev) { try { var txt = ev.target.result; if (file.name.endsWith('.json')) { var d = JSON.parse(txt); if (d.maps) { maps = d.maps; features = d.features || []; CATS = d.cats || []; if (d.roles) roles = d.roles; activeMapId = maps[0] ? maps[0].id : ''; } save(); renderMapFeatures(); fitToFeatures(); updateStats(); applyPermissionUI(); closeSheet('import'); toast('가져오기 완료', 's'); } else { importGeoFile(txt, file.name); } } catch (err) { toast('파일 오류', 'e'); } }; r.readAsText(file); e.target.value = ''; }
function importGeoFile(txt, fname) { var items = []; if (fname.endsWith('.kml')) { var doc = new DOMParser().parseFromString(txt, 'text/xml'); doc.querySelectorAll('Placemark').forEach(function (pm) { var name = (pm.querySelector('name') || {}).textContent || '장소'; var coord = pm.querySelector('coordinates'); if (coord) { var parts = coord.textContent.trim().split(','); if (parts.length >= 2) items.push({ title: name, lng: parseFloat(parts[0]), lat: parseFloat(parts[1]) }); } }); } else if (fname.endsWith('.gpx')) { var doc = new DOMParser().parseFromString(txt, 'text/xml'); doc.querySelectorAll('wpt').forEach(function (wpt) { items.push({ title: (wpt.querySelector('name') || {}).textContent || '장소', lat: parseFloat(wpt.getAttribute('lat') || 0), lng: parseFloat(wpt.getAttribute('lon') || 0) }); }); } if (!items.length) { toast('장소 없음', 'w'); return; } var m = { id: uid('m'), kind: 'my', title: '가져온 지도', desc: items.length + '개', theme: '#4F86C6', ver: 1 }; maps.push(m); activeMapId = m.id; items.forEach(function (it) { features.push({ id: uid('f'), mapId: m.id, type: 'pin', title: it.title, category: '_none', tags: [], emoji: '📍', isHl: false, geojson: { type: "Feature", geometry: { type: "Point", coordinates: [it.lng, it.lat] }, properties: {} }, logs: [] }); }); save(); closeSheet('import'); renderMapFeatures(); fitToFeatures(); updateStats(); applyPermissionUI(); toast(items.length + '개 가져오기 완료!', 's'); }
function resetAll() { showConfirm('초기화', '모든 데이터 삭제?', function () { localStorage.removeItem('loca_v2'); location.reload(); }); }

// ══════ POSTER ══════
function buildFrameListHtml(cat) { var list = ALL_FRAMES.filter(function (f) { return cat === 'all' || f.cat === cat; }); return list.map(function (fr) { var sel = fr.id === selectedFrame; return '<div data-action="pickframe" data-fid="' + fr.id + '" style="min-width:72px;padding:8px 6px;border-radius:14px;border:2px solid ' + (sel ? 'var(--pri)' : 'var(--bdr)') + ';background:' + (sel ? 'var(--pri-lt)' : '#fff') + ';text-align:center;cursor:pointer;flex-shrink:0"><div style="font-size:20px">' + fr.emoji + '</div><div style="font-size:9px;font-weight:900;margin-top:2px;color:' + (sel ? 'var(--pri)' : 'var(--txt2)') + '">' + fr.label + '</div><div style="font-size:7px;color:var(--mut);font-weight:700">' + fr.desc + '</div></div>'; }).join(''); }
function filterFrames(cat) { frameCatFilter = cat; document.querySelectorAll('#frame-cat-tabs .chip').forEach(function (c) { c.classList.toggle('on', c.dataset.fcat === cat); }); document.getElementById('frame-list').innerHTML = buildFrameListHtml(cat); }
function pickFrame(fid) { selectedFrame = fid; document.getElementById('frame-list').innerHTML = buildFrameListHtml(frameCatFilter); applyFramePreview(); }
function applyFramePreview() {
  var am = getActiveMap(); if (!am) return; var preview = document.getElementById('poster-preview'); if (!preview) return;
  var tc = am.theme || '#C9686E'; var deco = FRAME_DECOS[selectedFrame]; var isDk = selectedFrame === 'film' || selectedFrame === 'dark' || selectedFrame === 'neon' || (deco && deco.dark);
  var styles = { default: 'background:linear-gradient(165deg,#fff,' + tc + '08,' + tc + '15);border:2px solid ' + tc + '33', polaroid: 'background:#fff;border:none;box-shadow:0 4px 20px rgba(0,0,0,.15)', film: 'background:#1a1a1a;border:2px solid #333', minimal: 'background:#fff;border:1px solid #e0e0e0', dark: 'background:linear-gradient(165deg,#1a1a2e,#16213e,#0f3460);border:2px solid #333', vintage: 'background:linear-gradient(165deg,#f5e6d3,#e8d5b7,#d4c4a8);border:2px solid #c4a882', stamp: 'background:#fff;border:3px dashed ' + tc, neon: 'background:linear-gradient(165deg,#0a0a1a,#1a0a2e,#0a1a2e);border:2px solid ' + tc };
  if (deco) preview.style.cssText = 'width:100%;aspect-ratio:4/5;border-radius:22px;display:flex;flex-direction:column;overflow:hidden;position:relative;z-index:1;background:linear-gradient(165deg,' + deco.bg + ',' + deco.bg2 + ');border:2.5px solid ' + deco.border;
  else preview.style.cssText = 'width:100%;aspect-ratio:4/5;border-radius:22px;display:flex;flex-direction:column;overflow:hidden;position:relative;z-index:1;' + (styles[selectedFrame] || styles['default']);
  var tEl = document.getElementById('poster-title-el'); var lEl = document.getElementById('poster-logo-el');
  if (tEl) tEl.style.color = isDk ? '#fff' : ''; if (lEl) lEl.style.color = isDk ? tc : tc;
}
// Sticker system
function addStickerToLayer(emoji) { var layer = document.getElementById(stickerTarget); if (!layer) return; var rect = layer.getBoundingClientRect(); if (rect.width < 10) return; var x = (rect.width / 2) - 15, y = (rect.height / 2) - 15, rot = Math.round(Math.random() * 30 - 15), size = 30; var el = document.createElement('div'); el.textContent = emoji; el.dataset.rot = String(rot); el.dataset.size = String(size); el.style.cssText = 'position:absolute;left:' + x + 'px;top:' + y + 'px;font-size:' + size + 'px;cursor:grab;user-select:none;pointer-events:auto;z-index:20;filter:drop-shadow(0 2px 4px rgba(0,0,0,.15));line-height:1;transform:rotate(' + rot + 'deg);transition:transform .15s,font-size .15s'; el.addEventListener('mousedown', function (ev) { stickerDragStart(ev, el); }); el.addEventListener('touchstart', function (ev) { stickerDragStart(ev, el); }, { passive: false }); el.addEventListener('dblclick', function () { removeStickerEl(el); }); var lt = 0; el.addEventListener('touchend', function () { var now = Date.now(); if (now - lt < 300) removeStickerEl(el); lt = now; }); el.addEventListener('click', function (ev) { ev.stopPropagation(); selectSticker(el); }); layer.appendChild(el); selectSticker(el); }
...
function showShareSheet() {
  var am = getActiveMap(); if (!am) return; var fs = getMapFeatures(); var pins = fs.filter(function (f) { return f.type === 'pin'; }); var hlPins = pins.filter(function (f) { return f.isHl; }); var center = lmap.getCenter(); var zoom = lmap.getZoom(); var body = document.getElementById('poster-body');
  ...
  h += '<div id="poster-outer" style="width:100%;position:relative;margin-bottom:12px"><div id="poster-preview" style="width:100%;aspect-ratio:4/5;background:linear-gradient(165deg,#fff,' + am.theme + '08,' + am.theme + '15);border:2px solid ' + am.theme + '33;border-radius:22px;display:flex;flex-direction:column;overflow:hidden;position:relative;z-index:1"><div style="padding:18px 18px 0;text-align:center;flex-shrink:0"><div id="poster-logo-el" style="font-size:10px;font-weight:900;color:' + am.theme + ';letter-spacing:2px">LOCA</div><div id="poster-title-el" style="font-size:18px;font-weight:900;margin-top:3px">' + am.title + '</div>' + (am.desc ? '<div style="font-size:10px;color:var(--mut);font-weight:700;margin-top:2px">' + am.desc + '</div>' : '') + '</div><div id="poster-map-wrap" style="flex:1;margin:10px 14px 0;border-radius:14px;overflow:hidden;border:1px solid ' + am.theme + '33;min-height:0;position:relative"></div>';
  ...
  h += '<div style="text-align:center;padding:4px 0 12px;font-size:7px;color:var(--mut);font-weight:700">Made with LOCA 🗺</div></div><div id="sticker-layer" style="position:absolute;top:0;left:0;width:100%;height:100%;z-index:50;pointer-events:none;overflow:visible"></div></div>';
  h += '<div style="background:var(--srf);border:1px solid var(--bdr);border-radius:18px;padding:10px 12px;margin-bottom:10px"><div style="font-size:12px;font-weight:900;margin-bottom:6px">🖼️ 프레임</div><div style="display:flex;gap:4px;margin-bottom:8px" id="frame-cat-tabs"><span class="chip on" data-fcat="all" onclick="filterFrames(\'all\')">전체</span><span class="chip" data-fcat="basic" onclick="filterFrames(\'basic\')">기본</span><span class="chip" data-fcat="theme" onclick="filterFrames(\'theme\')">테마</span></div><div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:4px" id="frame-list">' + buildFrameListHtml('all') + '</div></div>';
  h += '<div style="background:var(--srf);border:1px solid var(--bdr);border-radius:18px;padding:10px 12px;margin-bottom:10px"><div style="font-size:12px;font-weight:900;margin-bottom:6px">🎨 스티커</div><div style="display:flex;gap:4px;margin-bottom:8px;overflow-x:auto" id="sticker-cat-tabs">' + STICKER_CATS.map(function (c, i) { return '<span class="chip ' + (i === 0 ? 'on' : '') + '" data-stab="' + i + '" onclick="switchStickerCat(' + i + ')">' + c.label + '</span>'; }).join('') + '</div><div id="sticker-items" style="display:flex;gap:4px;flex-wrap:wrap"></div><div style="font-size:9px;color:var(--mut);font-weight:700;margin-top:6px">탭→추가 · 드래그→이동 · 더블클릭→삭제</div><div id="sticker-edit" style="display:none;margin-top:8px;padding-top:8px;border-top:1px solid var(--bdr)"><div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span id="sticker-edit-preview" style="font-size:24px;line-height:1"></span><span style="font-size:11px;font-weight:900;color:var(--pri)">선택됨</span></div><div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><span style="font-size:10px;font-weight:800;width:32px">크기</span><input type="range" id="sticker-size" min="16" max="64" value="30" style="flex:1;accent-color:var(--pri)" oninput="onStickerSize(this.value)"><span id="sticker-size-val" style="font-size:10px;font-weight:800;color:var(--mut);width:28px;text-align:right">30</span></div><div style="display:flex;align-items:center;gap:8px"><span style="font-size:10px;font-weight:800;width:32px">회전</span><input type="range" id="sticker-rot" min="-180" max="180" value="0" style="flex:1;accent-color:var(--sec)" oninput="onStickerRot(this.value)"><span id="sticker-rot-val" style="font-size:10px;font-weight:800;color:var(--mut);width:28px;text-align:right">0°</span></div></div></div>';
  h += '<div class="gap-8"><button class="btn btn-pri" style="flex:1" onclick="savePosterAsImage(true)">💾 QR 포함</button><button class="btn btn-sec" style="flex:1" onclick="savePosterAsImage(false)">💾 QR 없이</button></div>';
  body.innerHTML = h; openSheet('poster'); stickerTarget = 'sticker-layer'; activeSticker = null;
...
function savePosterAsImage(includeQR) {
  var am = getActiveMap(); if (!am) return; var fs = getMapFeatures(); var pins = fs.filter(function (f) { return f.type === 'pin'; }); var hlPins = pins.filter(function (f) { return f.isHl; });
  if (activeSticker) activeSticker.style.outline = 'none'; toast('이미지 생성 중…', 'w');
  var W = 1080, H = 1350, cv = document.createElement('canvas'); cv.width = W; cv.height = H; var ctx = cv.getContext('2d');
  var tc = am.theme || '#C9686E'; var fr = selectedFrame; var deco = FRAME_DECOS[fr]; var isDark = fr === 'film' || fr === 'dark' || fr === 'neon' || (deco && deco.dark);
  var titleColor = isDark ? '#fff' : '#2C2925'; var logoColor = isDark ? tc : tc;
  // BG
  if (deco) { var gt = ctx.createLinearGradient(0, 0, W, H); gt.addColorStop(0, deco.bg); gt.addColorStop(1, deco.bg2); ctx.fillStyle = gt; roundRect(ctx, 0, 0, W, H, 40, true); ctx.strokeStyle = deco.border; ctx.lineWidth = 5; roundRect(ctx, 3, 3, W - 6, H - 6, 38, false, true); var emjs = deco.emojis; var positions = []; for (var ei = 0; ei < 8; ei++) { positions.push({ x: 60 + ei * (W - 120) / 7, y: 12 + Math.random() * 30, r: Math.random() * 40 - 20, s: 28 + Math.random() * 18 }); positions.push({ x: 60 + ei * (W - 120) / 7, y: H - 45 + Math.random() * 25, r: Math.random() * 40 - 20, s: 28 + Math.random() * 18 }); } for (var ei = 0; ei < 6; ei++) { positions.push({ x: 5 + Math.random() * 25, y: 120 + ei * (H - 240) / 5, r: Math.random() * 40 - 20, s: 24 + Math.random() * 16 }); positions.push({ x: W - 35 + Math.random() * 20, y: 120 + ei * (H - 240) / 5, r: Math.random() * 40 - 20, s: 24 + Math.random() * 16 }); } ctx.globalAlpha = 0.6; positions.forEach(function (p) { ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.r * Math.PI / 180); ctx.font = Math.round(p.s) + 'px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(emjs[Math.floor(Math.random() * emjs.length)], 0, 0); ctx.restore(); }); ctx.globalAlpha = 1; logoColor = deco.accent; }
  ...
  if (allC.length) {
    var mnLng = Infinity, mxLng = -Infinity, mnLat = Infinity, mxLat = -Infinity; allC.forEach(function (c) { if (c[0] < mnLng) mnLng = c[0]; if (c[0] > mxLng) mxLng = c[0]; if (c[1] < mnLat) mnLat = c[1]; if (c[1] > mxLat) mxLat = c[1]; }); var pf = .15; var dLng = (mxLng - mnLng) * pf || .005; var dLat = (mxLat - mnLat) * pf || .005; mnLng -= dLng; mxLng += dLng; mnLat -= dLat; mxLat += dLat;
  ...
  // Export
  var imgUrl = cv.toDataURL('image/png'); try { var a = document.createElement('a'); a.download = 'LOCA_' + iso() + '.png'; a.href = imgUrl; a.click(); } catch (e) { }
  ...
  sd.innerHTML = '<div style="color:#fff;font-size:14px;font-weight:900;margin-bottom:12px">📸 길게 눌러 저장</div><img src="' + imgUrl + '" style="max-width:85%;max-height:70vh;border-radius:16px;box-shadow:0 12px 40px rgba(0,0,0,.5)"><button class="btn" style="margin-top:14px;background:#fff;color:#333" onclick="document.getElementById(\'poster-save-result\').remove()">닫기</button>';
  document.body.appendChild(sd); toast('포스터 완료!', 's');
}

// ══════ INIT ══════
function init() {
  loadData(); initMap(); renderMapFeatures(); fitToFeatures(); updateStats(); applyPermissionUI(); renderFilterChips();
  maps.forEach(function (m) { if (m.kind === 'our' && !roles[m.id]) roles[m.id] = 'owner'; });
}
